...
pr1<-ggplot(dend_dataRef$segments)+
  ...
  geom_point(data = dend_dataRef2,aes(x,y,col=pop),size=3)+
  geom_segment(aes(x = x, y = y, xend = xend, yend = yend))+theme_light()+xlab("")+ylab("epigenetic distance")+
  scale_colour_manual(values = colorBlindBlack8)#+ggtitle("A")
...
g <- ggarrange(pr2,pr1, ncol=2, nrow=1, widths=c(6,6), align="h", labels="AUTO", common.legend = TRUE, legend="right", legend.grob=get_legend(pr1))
g
...
g
ggsave("bothTrees.tiff",g,height=4.5,width=9,dpi="retina")
